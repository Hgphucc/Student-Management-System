...
void xoa_xuong_dong(char x[]) {
    size_t len = strlen(x);      //tinh do dai chuoi khong lay  /0
    if (len > 0 && x[len - 1] == '\n') {
        x[len - 1] = '\0';
    }
}
...
void nhap_mot_sinh_vien(SinhVien* sv) {
    ...
    sv->next = NULL; // Khoi tao con tro next la NULL xem node cuoi danh sach
}
...
void hien_thi_danh_sach(SinhVien* head) {
    SinhVien* p = head;// Duyet tu dau danh sach
...
void xoa_sinh_vien(char mssv[]) {
    SinhVien* p = head;
    SinhVien* truoc = NULL; //con tro de luu phan tu truoc phan tu hien tai
...
SinhVien* tim_theo_mssv(char mssv[]) {
    ...
    while (p != NULL) {
        if (strcmp(p->mssv, mssv) == 0) {
            return p;  // Tra ve dia chi node tim duoc
        }
        p = p->next;
    }
    return NULL; // Khong tim thay thi tra NULL
}
void hien_thi_diem_cao_nhat(SinhVien* head) {
    ...
    // Kiem tra co sinh vien nao co GPA cao nhat không
    if (maxGPA == 0) {
        printf("Khong co sinh vien co GPA cao.\n");
        return;
    }

    // In cac SV co GPA cao nhat
    ...
    p = head;
    int found = 0;  // Bien kiem tra neu co sinh vien nao co GPA cao nhat
    while (p != NULL) {
        if (p->GPA == maxGPA) {
            printf("| %-10s | %-29s | %-13s | %6.2f |\n",
                p->mssv, p->hoTen, p->lopHoc, p->GPA);
    ...
    for (i = head; i != NULL; i = i->next) {
        for (j = i->next; j != NULL; j = j->next) {
            if (strcmp(layTen(i->hoTen), layTen(j->hoTen)) > 0) {
                // Hoan doi toan bo du lieu sinh vien
                SinhVien temp = *i;
                *i = *j;
                *j = temp;

                // Khoi phuc lien ket next (vi *i va *j bi trao)
